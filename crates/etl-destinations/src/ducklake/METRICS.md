...
Streaming connections attach with `DATA_INLINING_ROW_LIMIT = 512`, so a batch
below that lands in the catalog instead of a tiny Parquet file, while a larger
batch goes straight to Parquet where it carries column statistics for pruning.
