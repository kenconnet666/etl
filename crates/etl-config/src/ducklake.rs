...
pub enum ParseDucklakeUrlError {
    ...
    #[error("DuckLake data path must use the s3://, gs:// or file:// scheme, got {0}://")]
...
/// Parses a DuckLake data path.
///
/// Object storage is the usual choice, but a local path is accepted too, which
/// keeps benchmarks and local runs independent of an S3 service.
pub fn parse_ducklake_data_path(value: &str) -> Result<Url, ParseDucklakeUrlError> {
    let url = parse_ducklake_url(value)?;

    match url.scheme() {
        "s3" | "gs" | "file" => Ok(url),
        scheme => Err(ParseDucklakeUrlError::UnsupportedDataPathScheme(scheme.to_owned())),
    }
}
...
mod tests {
    ...
    #[test]
    fn parse_ducklake_data_path_accepts_s3_urls() {
        let url = parse_ducklake_data_path("s3://bucket/path").unwrap();

        assert_eq!(url.as_str(), "s3://bucket/path");
    }

    #[test]
    fn parse_ducklake_data_path_accepts_local_paths() {
        let url = parse_ducklake_data_path("/var/lib/lake").unwrap();
        assert_eq!(url.scheme(), "file");
    }

    #[test]
    fn parse_ducklake_data_path_rejects_other_schemes() {
        let error = parse_ducklake_data_path("http://example.com/lake").unwrap_err();
        assert!(matches!(error, ParseDucklakeUrlError::UnsupportedDataPathScheme(_)));
    }
